---
title: "Figure 5 (male-specific variant) v5"
subtitle: "Main-figure panels bordered red; supplementary views unbordered with full descriptions."
geometry: margin=1in
header-includes:
  - \usepackage{graphicx}
  - \usepackage{float}
...
**(B') Log2 ppk23/ppk25 ratio per AN09B017 variant.** Bar quantification of the selectivity spectrum from (B), variants ordered by ratio. Positive bars = male-biased variants, negative = female-biased.
...
**(C') ppk23 vs ppk25 per-subtype scatter.** Each point one mAL_m subtype. Quadrants identify ppk23-only, ppk25-only, both-active, and both-suppressed subtypes. Complements the bar view in (C) by showing the joint distribution.
...
**(E') E/I balance scatter.** x = ppk23 E/I ratio, y = ppk25 E/I ratio per subtype; ratio < 1 means inhibition dominates that channel. Mean balance: ppk23 = 0.314, ppk25 = 0.424. Shows that small weight changes on the inhibitory arm could flip a subtype's polarity on either channel.
