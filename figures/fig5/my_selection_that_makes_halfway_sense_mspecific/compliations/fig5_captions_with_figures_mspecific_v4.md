---
title: "Figure 5 (male-specific variant) v4"
subtitle: "Panel captions with figures. Main-figure panels bordered in red; supplementary views unbordered and labeled (SUPP)."
geometry: margin=1in
header-includes:
  - \usepackage{graphicx}
  - \usepackage{float}
...
**(A-SUPP-1) Panel A recomputed with widest-path raw synapse counts.** Cell = signed max path-bottleneck synapse count (exc - inh), picking the single widest path per valence. Controls for path-count and normalization artifacts in (A).

\newpage

![](../single panels/panel_A_input_profiles_max_flow.pdf)

**(A-SUPP-2) Panel A recomputed with max-flow raw synapses.** Cell = `max_flow_exc - max_flow_inh` on the subgraph induced by all cached paths; edge capacities = raw synapse counts; virtual super-source connected to every source sensillum. Accounts for parallel-route redundancy and shared bottleneck edges. **Caveat:** standard max-flow does not penalize path length; a long weak chain can contribute the same as a short strong one if their bottlenecks match.

\newpage

![](../single panels/panel_A_input_profiles_max_flow_normed.pdf)

**(A-SUPP-3) Panel A max-flow with input-normalized edge capacities.** Same procedure as (A-SUPP-2) but capacities from `adj.matrix` (input-normalized = fraction of postsynaptic input budget). Units match panel A's path-strength product sums. Same path-length caveat as (A-SUPP-2).

\newpage

![](../single panels/panel_A_input_profiles_topK_sum.pdf)

**(A-SUPP-4) Sum of top-10 widest bottlenecks per valence.** For each (modality, mAL, valence) triple, pick the 10 paths with highest bottleneck and sum their bottleneck synapse counts; cell = exc sum - inh sum. Treats routes as parallel pipes carrying independent signals; double-counts when paths share bottleneck edges.

\newpage

![](../single panels/panel_A_input_profiles_topK_mean.pdf)

**(A-SUPP-5) Mean of top-10 widest bottlenecks per valence.** Same top-10 pick as (A-SUPP-4), but mean instead of sum. "Typical capacity of the strongest K routes"; less noisy than the sum and deflates when multiple good routes exist (information you may want reflected).
...
\mainpanel{../single panels/panel_B_selectivity_ratio.pdf}
...
**(K) Relays route channels to specific mAL subsets.** Input-normalized connectivity of AN09B017a-g + AN05B035 to the 16 male-specific mAL_m subtypes. Completes the labeled-line motif: M-cell (ppk23) -> AN09B017 subset -> specific mAL_m; F-cell (ppk25) -> different AN09B017 subset -> different mAL_m.
...
**(G') Lateral inhibition hubs.** mAL_m1 and mAL_m8 are dominant inhibitory outputs; mAL_m5b/c and mAL_m2b receive heaviest lateral inhibition. Drive magnitude does not predict inhibition received (rho = 0.32).

\newpage

![](../single panels/panel_G_lateral_heatmap_normed.pdf)

**(G-SUPP) Panel G recomputed with input-normalized weights.** mAL<->mAL signed input fractions. Complements the raw-synapse (G).
...
**(F') Same scenarios under the signal-flow model.** Iterative nonlinear propagation (tanh saturation + rectification). Same scenario-discriminators as (F).
...
**(D') Total mAL drive per P1 subtype.** Stacked bars summing signed mAL_m -> P1 weights.

\newpage

![](../single panels/panel_D_mal_to_p1_normed.pdf)

**(D-SUPP) Panel D recomputed with input-normalized weights.** Each cell = fraction of the P1's total input, signed by mAL neurotransmitter. Complements the raw-synapse (D).
...
**(J') Predicted P1 drive under the three encounter scenarios.** Stacked bars per P1 subtype under Female (ppk25), cVA+male, cVA+female.
...
**(I') Model agreement extends from mAL to P1.** Mean path-based vs mean signal-flow drive per subtype, both populations, point size = joint spread across scenarios.

\newpage

![](../single panels/panel_I_scenario_heatmap_supp.pdf)

**(I-SUPP-a) mAL 3-scenario signal-flow heatmap.**

\newpage

![](../single panels/panel_I_p1_scenario_heatmap_supp.pdf)

**(I-SUPP-b) P1 3-scenario signal-flow heatmap.**

\newpage

![](../single panels/panel_I_p1_combined_supp.pdf)

**(I-SUPP-c) P1 version of the Panel I 2x2 grid.**

\newpage

![](../single panels/panel_I_mal_scenario_spread_bar_supp.pdf)

**(I-SUPP-d) mAL scenario spread bar.** Per-mAL subtype spread across scenarios, bar color = argmax scenario.

\newpage

![](../single panels/panel_I_mal_scenario_trajectories_supp.pdf)

**(I-SUPP-e) Top-15 most scenario-discriminating mAL subtypes.**

\newpage

![](../single panels/panel_I_p1_scenario_spread_bar_supp.pdf)

**(I-SUPP-f) P1 scenario spread bar.**

\newpage

![](../single panels/panel_I_p1_scenario_spread_scatter_supp.pdf)

**(I-SUPP-g) P1 mean-drive vs spread.**

\newpage

![](../single panels/panel_I_p1_scenario_trajectories_supp.pdf)

**(I-SUPP-h) Top-15 most scenario-discriminating P1 subtypes.**

\newpage

![](../single panels/panel_I_mal_cva_mvf_bar_supp.pdf)

**(I-SUPP-i) Focused mAL cVA+male vs cVA+female spread.** Isolates ppk25 vs ppk23 sensitivity under shared cVA background.
