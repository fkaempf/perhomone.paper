...
### 0 Define save path 
```{r}
plot.path.png <- "/Users/fkampf/Documents/pheromone.paper/analyses/strongest_path/plots/remove.dimorphism/png/"
plot.path.pdf <- "/Users/fkampf/Documents/pheromone.paper/analyses/strongest_path/plots/remove.dimorphism/pdf/"
```
...
df_all.all.VA1v <- df_all.all.VA1v %>%
  rowwise() %>%
  filter(lists_overlap(nodes, sex.dimorphic.types)) %>%
  ungroup()
...
used <- df_all.all.ppk23.excluded

t_res <- t.test(log(strength) ~ dataset, data = used)
pval <- unname(t_res$p.value)
ann <- data.frame(
  group1      = "PPN1",
  group2      = "vAB3",
  p           = pval,
  y.position  = max(used$strength, na.rm = TRUE) * 1.12
)

p<-ggplot(used, aes(x = dataset, y = strength)) +
  geom_boxplot(outlier.shape = NA, fill = "lightgray") +
  geom_jitter(width = 0.2, height = 0, alpha = 0.4, color = "blue") +
  theme_bw() +
...
p <- ggplot(used, aes(x = dataset, y = factor(hops))) +
  geom_count() +
  scale_size_area(max_size = 6) +
  theme_bw() +
  labs(y = "hops (discrete)") +
