---
geometry: "margin=0.35in,includefoot,footskip=18pt"
fontsize: 10pt
header-includes:
  - \usepackage{graphicx}
  - \usepackage{float}
  ...
  - \usepackage{fancyhdr}
  - \fancypagestyle{figpage}{\fancyhf{}\renewcommand{\headrulewidth}{0pt}\renewcommand{\footrulewidth}{0pt}\fancyfoot[C]{\small\thepage}}
  - \pagestyle{figpage}
  - \AtBeginDocument{\small\pagestyle{figpage}\thispagestyle{figpage}}
  - \makeatletter\let\ps@plain\ps@figpage\makeatother
  ...
  - \newcommand{\mainpanel}[1]{\begin{figure}[H]\centering\adjustbox{cfbox=red 2pt 0pt,max width=\linewidth,max height=0.78\textheight,keepaspectratio}{\includegraphics{#1}}\end{figure}\vspace{-0.5em}}
  - \newcommand{\suppfig}[1]{\begin{figure}[H]\centering\adjustbox{max width=\linewidth,max height=0.78\textheight,keepaspectratio}{\includegraphics{#1}}\end{figure}\vspace{-0.5em}}
---

\null\vfill
\mainpanel{../single panels cropped/panel_A_input_profiles_raw.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_A_input_profiles_raw_synapses.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_A_input_profiles_max_flow.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_A_input_profiles_max_flow_normed.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_A_input_profiles_topK_sum.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_A_input_profiles_topK_mean.pdf}
...
\null\vfill
\mainpanel{../single panels cropped/panel_B_an09b017_selectivity.pdf}

**(B) AN09B017 ascending neurons tile a ppk23/ppk25 selectivity spectrum.** Scatter of ppk23 vs ppk25 input per AN09B017 variant (a-g). ppk23 = M-cell gustatory sensory neurons innervating male contact sensilla; ppk25 = F-cell gustatory sensory neurons innervating female contact sensilla. Variants span a broad ppk23-to-ppk25 bias range, with AN09B017a the most ppk23-preferring and AN09B017g the most ppk25-preferring. Channel separation of male vs female contact pheromone begins in the ascending relay layer.

\vfill\null
\clearpage

\null\vfill
\mainpanel{../single panels cropped/panel_B_an09_dominance_vs_ppk_asymmetry_v6.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_B_selectivity_ratio.pdf}
...
\null\vfill
\mainpanel{../single panels cropped/panel_C_ppk23_sign_reversal.pdf}
...
\null\vfill
\mainpanel{../single panels cropped/panel_C_ppk23_vs_ppk25_scatter.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_M_example_traces_ppk25_supp.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_M_mal_by_si_input_supp.pdf}

**(M-SUPP-c) mAL_m subtypes ordered by GABA SI-pool input.** 16 male-specific mAL_m subtypes ranked by total SI-driven path strength, stacked by SI pool member identity. Bars come from a top-50 path computation with the SI as source and mAL_m as target, so the contact-sensilla -> SI prefix is dropped and bar height reflects each SI's downstream reach. Top targets: mAL_m1 (0.177), mAL_m5a (0.173), mAL_m2a (0.122). Channel-agnostic complement to (M'), which attributes channel bias on the input side.

\vfill\null
\clearpage

\null\vfill
\mainpanel{../single panels cropped/panel_E_valence_bars.pdf}
...
\null\vfill
\mainpanel{../single panels cropped/panel_E_ei_balance_scatter.pdf}

**(E') E/I balance scatter.** Each mAL_m subtype plotted by ppk23 (x) and ppk25 (y) E/I balance index = `(exc - inh) / (exc + inh)`, range [-1, +1]. All 16 male-specific subtypes are net excitatory on both channels (mean balance ppk23 = 0.314, ppk25 = 0.424); ppk25 is systematically less inhibition-weighted than ppk23, consistent with AN05B035 being ppk23-biased at the pool's top throughput rank (see M'). mAL_m5a (ppk23 balance 0.04) sits one modest weight change from flipping into ppk23 inhibition.

\vfill\null
\clearpage

\null\vfill
\mainpanel{../single panels cropped/panel_K_heatmap.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_K_heatmap_raw.pdf}
...
\null\vfill
\mainpanel{../single panels cropped/panel_G_lateral_heatmap.pdf}
...
\null\vfill
\mainpanel{../single panels cropped/panel_G_hub_analysis.pdf}

**(G') Lateral inhibition hubs.** Per-subtype ranking of inhibitory output and input. mAL_m1, mAL_m5a and mAL_m4 are the dominant inhibitory outputs (top-3 by summed inhibitory output); mAL_m5b, mAL_m5c and mAL_m2b receive the heaviest lateral inhibition. Drive magnitude only weakly predicts inhibition received (Spearman rho = 0.32), so lateral competition sharpens the code largely independently of input strength.

\vfill\null
\clearpage

\null\vfill
\suppfig{../single panels cropped/panel_G_lateral_heatmap_normed.pdf}
...
\null\vfill
\mainpanel{../single panels cropped/panel_L_mal_and_p1_ppk_selectivity.pdf}
...
\null\vfill
\mainpanel{../single panels cropped/panel_L_mal_and_p1_sf_ppk_selectivity.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_L_mal_ppk_selectivity_supp.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_L_mal_sf_ppk_selectivity_supp.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_L_p1_ppk_selectivity_supp.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_L_p1_sf_ppk_selectivity_supp.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_D_mal_to_p1_normed.pdf}
...
\null\vfill
\mainpanel{../single panels cropped/panel_J_cva_delta_scatter.pdf}
...
\null\vfill
\mainpanel{../single panels cropped/panel_J_per_p1_bars.pdf}
...
\null\vfill
\mainpanel{../single panels cropped/panel_I_combined.pdf}
...
\null\vfill
\mainpanel{../single panels cropped/panel_I_mal_and_p1_spread_scatter.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_I_scenario_heatmap_supp.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_I_p1_scenario_heatmap_supp.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_I_p1_combined_supp.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_I_mal_scenario_spread_bar_supp.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_I_mal_scenario_trajectories_supp.pdf}

**(I-SUPP-e) Top-15 most scenario-discriminating mAL subtypes.** For the 15 mAL_m with largest spread, plot drive under Female -> cVA+male -> cVA+female as a connected line (one line per subtype, labeled). Shows the direction of scenario-induced shifts: the Female -> cVA+male transition adds DA1 and swaps ppk25 for ppk23, while the cVA+male -> cVA+female transition keeps DA1 and swaps ppk23 for ppk25; subtypes that flip sign across scenarios cross zero.

\vfill\null
\clearpage

\null\vfill
\suppfig{../single panels cropped/panel_I_p1_scenario_spread_bar_supp.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_I_p1_scenario_spread_scatter_supp.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_I_p1_scenario_trajectories_supp.pdf}
...
\null\vfill
\suppfig{../single panels cropped/panel_I_mal_cva_mvf_bar_supp.pdf}
...
\null\vfill
\mainpanel{../single panels cropped/panel_N_an05b102_selectivity.pdf}

**(N) AN05B102 is a parallel cholinergic ascending relay for ppk23/ppk25.** Scatter of ppk23 vs ppk25 input (input-normalized fraction) per AN05B102 variant (a-d; all acetylcholine). Parallel to (B) but for the AN05B102 family. AN05B102 appears in Figure 5's underlying path feather (approximately 3,233 of 18,500 cached ppk23 -> mAL paths traverse an AN05B102 variant) but was not rendered in (B) or (K) because those panels were scoped to the glutamatergic AN09B017 family. Panels B and N together cover the excitatory ascending-relay layer; (M) covers the GABAergic inhibitory arm.

\vfill\null
\clearpage

\null\vfill
\mainpanel{../single panels cropped/panel_N_an05b102_dominance_vs_ppk_asymmetry.pdf}

**(N') AN05B102 input composition per mAL vs ppk asymmetry.** Each mAL_m subtype is drawn as a pie chart at position (x = ppk23 path-based drive minus ppk25 path-based drive, y = % of mAL's total input from all AN05B102 variants combined). Pie slices show the fraction contributed by each AN05B102 variant (a-d). Soft quadrant backgrounds label interpretation regions: top-right = male-contact-driven + strong AN05B102 input; top-left = female-contact-driven + strong AN05B102 input; bottom halves = weak AN05B102 involvement. Dashed guide lines mark `ppk23 - ppk25 = 0` (channel-balanced) and the median AN05B102 coupling. Parallel to (B').

\vfill\null
\clearpage

\null\vfill
\suppfig{../single panels cropped/panel_N_an05b102_selectivity_ratio_supp.pdf}

**(N-SUPP) Log2 ppk23/ppk25 input ratio per AN05B102 variant.** Bar quantification of (N)'s selectivity, variants ordered by ratio. Positive bars = ppk23-preferring, negative = ppk25-preferring.

\vfill\null
\clearpage

**One-line figure caption.** The 16 male-specific mAL_m* subtypes encode pheromone encounter identity through differential relay routing (ppk23 / M-cell and ppk25 / F-cell) via parallel excitatory ascending-neuron families (AN09B017 glutamatergic in B; AN05B102 cholinergic in N), GABAergic sign inversion via the AN05B035-led SI pool (M), and lateral competition (G); delivering channel-specific gating of P1 courtship command neurons that propagates the selectivity downstream (A -> B -> C -> M -> E -> K -> G -> F -> L -> D -> J -> I -> N).
