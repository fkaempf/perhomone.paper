...
#conna = neuprint_login(dataset='male-cns:v1.0',server='https://neuprint-cns.janelia.org/')
#mcns.rois = neuprint_ROIs(dataset = 'male-cns:v1.0')
...
calculate_normed_adj_matrix <-function(connectivity,cell.or.type='type',pre.or.post='pre'){
  ...
  if(pre.or.post=='pre'){
    ...
    adj.matrix.normed.pre <- rowScale(adj.matrix)
    colnames(adj.matrix.normed.pre) <- colnames(adj.matrix)
    rownames(adj.matrix.normed.pre) <- rownames(adj.matrix) # nolint # nolint: object_name_linter.
