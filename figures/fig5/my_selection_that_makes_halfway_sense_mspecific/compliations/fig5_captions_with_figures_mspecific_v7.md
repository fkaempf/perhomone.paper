---
title: "Figure 5 (male-specific variant) v7"
subtitle: "Main-figure panels bordered red; supplementary views unbordered with full descriptions. Panel M added as the mechanistic zoom-in on the GABA sign-inverter pool between E and G; story arc retained after explicit rethink because mechanism panels (M, G) sit most naturally between the E/I observation (C, E) and the integrated population signature (F)."
geometry: margin=1in
header-includes:
  - \usepackage{graphicx}
  - \usepackage{float}
...
**Story arc:** A -> B -> K -> C -> E -> M -> G -> F -> L -> D -> J -> I.
